...
library(ggplot2)
library(stringr)
user_path <- "/Users/danielaina/Documents/OMSBA 5210/DEC"
...
data_set3 <- data_set2 %>%
  ...
  ggplot(data_set2, mapping = aes(y = Publisher, x = NA_Sales, fill = Genre)) + labs(x = 'North American Sales', y = 'Publishers', title = 'Top Video Game Genres by Publishers') + scale_x_continuous(labels= scales::comma) + scale_color_brewer(palette="Set3") +
  geom_bar(stat = 'identity') +
  geom_point_interactive(aes(tooltip = tooltip), size = .05)


ggiraph(ggobj = data_set3)
