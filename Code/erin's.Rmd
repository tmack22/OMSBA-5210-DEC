...
tree <- treemap(tree_data_set,index=c("Region","Genre"),vSize="Sales",title= "Sales by Region and Genre") 
d3tree2(tree,rootname="Sales by Region and Genre")
```
...
renderggiraph({
  viz2 <- df2 %>% 
    mutate(tooltip = paste0(tooltip = paste0('Title: ', stringr::str_to_title(Name), 
                                             '\nHours Watched On Twitch: ', total_hours_watched, 
                                             '\nSales: ', total_sales,
                                             '\nAverage Peak Channels: ', avg_peak_channels))) %>%
    ggplot(df2, aes(x = total_hours_watched/1000000, y=total_sales/1000000, size = avg_peak_channels, color = Genre, tooltip= tooltip)) +
    geom_point(alpha = .7) +
