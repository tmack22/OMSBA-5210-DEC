...
df3 <- df %>% filter(Name %in% top_100$Name) %>% left_join(top_100,by=c("Name","Platform","Genre")) %>% subset(rank %in% c(1.0:100.0))
histo_df <- df3 %>%
  filter(Region %in% input$fil_region & Genre %in% input$fil_genre) %>% group_by(Name,Region,Genre) %>% mutate(all_sales = sum(Sales)) %>%
  ungroup()
viz3 <- histo_df %>% ggplot(aes(x = all_sales)) + geom_histogram()
ggiraph(ggobj = viz3)
})
```