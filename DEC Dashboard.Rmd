...
github_repo <- "https://github.com/tmack22/OMSBA-5210-DEC"

game_reviews <- read.csv("game_reviews.csv")
global_sales <- read.csv("global_sales.csv")
pc_game_sales <- read.csv("pc_game_sales.csv")
twitch_game_data <- read.csv("twitch_game_data.csv")
twitch_global_data <- read.csv("twitch_global_data.csv")
...
bar_data_set <- data_set2 %>% filter(Publisher %in% top_pub$Publisher) %>% 
                mutate(Sales_per_mil = Sales/1000000)

bar_plot <- ggplot(data=bar_data_set,mapping=aes(fill= Genre,x=Sales_per_mil,y=Publisher,tooltip = tooltip)) +
  geom_bar_interactive(stat="identity") + labs(x = 'Sales in Millions', y = 'Publishers', title = 'Top Video Game Genres by Publishers') + scale_color_brewer(palette="Set3")
...
renderggiraph({
  histo_df <- df3 %>% 
            filter(Region %in% input$fil_region & Genre %in% input$fil_genre) %>% group_by(Name,Region,Genre) %>% mutate(all_sales = sum(Sales)) %>% ungroup()
...
```{r bubble}
renderggiraph({
bubble_df <- df2 %>% select(-c("Region")) %>% filter(Genre %in% input$fil_genre) %>%
    mutate(tooltip = paste0(tooltip = paste0('Title: ', stringr::str_to_title(Name),
                                             '\nGenre: ',Genre,
...
### Metacritic Rating Distribution

```{r Histogram Ratings}
renderggiraph({
mscore_histo <- regression_data_set %>% filter(Region %in% input$fil_region & Genre %in% input$fil_genre)
...
```{r hist}
renderggiraph({
uscore_histo <- regression_data_set %>% filter(Region %in% input$fil_region & Genre %in% input$fil_genre)
