...
top_100 <- df %>% select(Name,Platform,total_sales) %>% unique() %>% arrange(desc(total_sales)) %>% head(n=100) %>%
  ungroup() %>%
  mutate(rank = row_number()) %>% select(-c("total_sales"))

df1 <- df %>% filter(Name %in% top_100$Name) %>% left_join(top_100,by=c("Name","Platform")) %>% subset(rank %in% c(1.0:100.0))
...
renderggiraph({
point_df <- df1 %>% 
            filter(Region %in% input$fil_region & Genre %in% input$fil_genre) %>% group_by(Name,Platform) %>% mutate(all_sales = sum(Sales)) %>% ungroup() %>%
...
```{r bubble, echo=FALSE}
renderggiraph({
bubble_df <- df2 %>% filter(Region %in% input$fil_region & Genre %in% input$fil_genre) %>%
    mutate(tooltip = paste0(tooltip = paste0('Title: ', stringr::str_to_title(Name), 
